...
ORM은 매우 편리하지만 만능은 아니며, 잘못 사용하면 오히려 성능 저하를 일으킬 수 있다.

1.  **학습 곡선 (Learning Curve):**  
    단순히 사용하는 것은 쉽지만 내부 동작 원리를 이해하지 못하면 성능 문제를 일으키기 쉽다.  
    특히 지연 로딩(Lazy Loading), 즉시 로딩(Eager Loading), 캐시 등 고급 개념을 제대로 활용하려면 깊이 있는 학습이 필요하다.

2.  **복잡한 쿼리 작성의 어려움:**  
    ORM은 일반적인 CRUD 작업에는 매우 효율적이지만 통계 처리나 복잡한 데이터 분석처럼 여러 테이블을 복잡하게 조인(JOIN)해야 하는 경우 ORM만으로는 한계가 있을 수 있다.  
    이 경우, 차라리 순수 SQL(Native SQL)을 작성하는 것이 더 효율적일 때가 많다.

3.  **성능 문제 발생 가능성 (N+1 문제 등):**  
    ORM이 자동으로 생성하는 SQL은 개발자가 의도하지 않은 비효율적인 쿼리일 수 있다. 가장 대표적인 예가 **'N+1 문제'** 입니다.
    *   (예: 게시글 목록(1)을 가져온 뒤, 각 게시글의 작성자(N) 정보를 얻기 위해 루프 안에서 쿼리를 N번 더 실행하는 문제)
    *   이러한 문제를 피하려면 Fetch Join 같은 ORM의 고급 기능을 이해하고 사용해야 한다.

4.  **추상화에 따른 한계:**  
    ORM은 데이터베이스의 모든 기능을 지원하지 못할 수 있다. 특정 데이터베이스에만 있는 고유한 함수나 기능을 사용해야 할 경우 ORM이 이를 지원하지 않으면 결국 순수 SQL을 사용해야 한다.